...
public class DictProcessor : IDictProcessor
{
    ...
    private static string EncodeNmsl(string text) =>
        text.SplitElement().Select(DictData.GetHanEmoji).Join("");

    private static string DecodeNmsl(string text) =>
        text.SplitElement().Select(DictData.GetEmojiHan).Join("");

    private static string EncodeSprk(string text) =>
        text.SplitElement().Select(DictData.GetHanSpark).Join("");

    private static string DecodeSprk(string text) =>
        text.SplitElement().Select(DictData.GetSparkHan).Join("");

    private static string EncodeDiff(string text) =>
        text.SplitElement().Select(DictData.GetUnicodeDiff).Join("");

    private static string EncodeTrad(string text) => ZhConverter.HansToHant(text);

    private static string DecodeTrad(string text) => ZhConverter.HantToHans(text);
}
